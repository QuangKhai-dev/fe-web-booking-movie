...
const TheaterShowTimes: React.FC<Props> = ({ movie }) => {
  ...
  const listTheaters = useMemo(
    () => movie?.lichChieu?.filter(item => item.thongTinRap.maHeThongRap === value),
    [value, movie]
  )
    ...
    <React.Fragment>
        ...
        <Box display="flex" alignItems="center" justifyContent="center" mb={4}>
          <Tabs value={value} onChange={handleChange} aria-label="disabled tabs example">
            {theatersSystemConnection?.map(theater => (
              <Tab
                key={`tab-theater-${theater.maHeThongRap}`}
                label={<img src={theater.logo} style={{ width: '50px', height: '50px' }} alt={theater.biDanh} />}
      ...
      <Fade delay={300}>
        <Box width="80%" sx={{ margin: '0 auto' }} mt={3}>
          ...
          {!theaterClusterLoading &&
            theatersClusterConnection?.map(item => (
              <Accordion key={item.maCumRap}>
                ...
                >
                  <Stack direction="row" spacing={0.5} alignItems="center">
                    ...
                    <img src="/images/ico_hc.png" style={{ width: 18, height: 18 }} alt="img" />
                  </Stack>
                </AccordionSummary>
                <AccordionDetails>
