...
const InformationMovie: React.FC<Props> = ({ movie }) => {
  ...
  return (
    <Box sx={{ width: '100%', position: 'relative', paddingBottom: '26.5%', borderRadius: 2, overflow: 'hidden' }}>
      <Fade delay={100}>
        <img
          src={movie?.hinhAnh}
              ...
              <Stack direction="row" justifyContent="space-between">
                <Stack spacing={1} pr={5}>
                  ...
                  <Typography fontWeight={400} color="common.white">
                    <span style={{ fontWeight: 700, marginRight: '10px' }}>{t('MOVIE_DETAILS.OPENING_DAY')}</span>
                    {movie?.ngayKhoiChieu && format(new Date(movie?.ngayKhoiChieu), 'dd/MM/yyy')}
                  </Typography>
                  <Typography fontWeight={400} color="common.white">
                    <span style={{ fontWeight: 700, marginRight: '10px' }}>{t('MOVIE_DETAILS.TIME')}</span>
                    {min && `${RANDOM_MIN[min as string]} ${t('COMMON.MIN')} - 9.1 IMDb`}
                  </Typography>
                  <Typography fontWeight={400} color="common.white">
                    <span style={{ fontWeight: 700, marginRight: '10px' }}>{t('MOVIE_DETAILS.DESCRIPTIONS')}</span>
                    {movie?.moTa}
                  </Typography>
                </Stack>
                <Box
